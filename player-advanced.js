...
function spxDetectSourceConfig(url) {
  if (!url) return { src: '', type: '' };

  // Si app.js expose déjà une détection unifiée, on l'utilise en priorité.
  try {
    if (window.SPX && typeof window.SPX.detectSource === 'function') {
      const cfg = window.SPX.detectSource(url);
      if (cfg && cfg.src && cfg.type) {
        return cfg;
      }
    }
  } catch (e) {
    console.warn('player-advanced: SPX.detectSource a échoué, fallback local.', e);
  }

  // Fallback local minimal, sans réécriture d'URL et sans hack CORS.
